<?php
    ...
    $select = trim(htmlspecialchars(strip_tags($_REQUEST["select"]), ENT_QUOTES, "UTF-8"));
    
    if (filter_var($temp, FILTER_VALIDATE_FLOAT)) {
        
        if ($select = Ce && $temp >= -273.15) {
            $resul = 32+(1.8*$temp);
            echo "El valor $temp en Fahrenheit, son $resul grados en Celsius";
        } else {
            echo "El valor introducido no puede menor que -459.67";
        }


        if ($select = Fa && $temp >= -459.67) {
            $resul = ($temp-32)/1.8;
            echo "El valor $temp en Fahrenheit, son $resul grados en Celsius";
        } else {
            echo "El valor introducido no puede menor que -459.67";
